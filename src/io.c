...
#endif
{
  vm_push_string_from_chars(vm, -1, "io");
  vm_push_string_from_chars(vm, -1, "StdIn");
  vm_push_userdata(vm, (userdata_t *) file_new(stdin));
  vm_push_string_from_chars(vm, -1, "StdOut");
  vm_push_userdata(vm, (userdata_t *) file_new(stdout));
  vm_push_string_from_chars(vm, -1, "StdErr");
  vm_push_userdata(vm, (userdata_t *) file_new(stderr));
  vm_push_string_from_chars(vm, -1, "SeekSet");
  vm_push_number(vm, SEEK_SET);
  vm_push_string_from_chars(vm, -1, "SeekCurrent");
  vm_push_number(vm, SEEK_CUR);
  vm_push_string_from_chars(vm, -1, "SeeEnd");
  vm_push_number(vm, SEEK_END);
  vm_push_string_from_chars(vm, -1, "open");
  vm_push_new_native(vm, "open", 2, &open_call);
  vm_push_string_from_chars(vm, -1, "close");
  vm_push_new_native(vm, "close", 1, &close_call);
  vm_push_string_from_chars(vm, -1, "popen");
  vm_push_new_native(vm, "popen", 2, &popen_call);
  vm_push_string_from_chars(vm, -1, "pclose");
  vm_push_new_native(vm, "pclose", 1, &pclose_call);
  vm_push_string_from_chars(vm, -1, "eof");
  vm_push_new_native(vm, "eof", 1, &eof_call);
  vm_push_string_from_chars(vm, -1, "flush");
  vm_push_new_native(vm, "flush", 1, &flush_call);
  vm_push_string_from_chars(vm, -1, "sync");
  vm_push_new_native(vm, "sync", 1, &sync_call);
  vm_push_string_from_chars(vm, -1, "tell");
  vm_push_new_native(vm, "tell", 1, &tell_call);
  vm_push_string_from_chars(vm, -1, "rewind");
  vm_push_new_native(vm, "rewind", 1, &rewind_call);
  vm_push_string_from_chars(vm, -1, "seek");
  vm_push_new_native(vm, "seek", 3, &seek_call);
  vm_push_string_from_chars(vm, -1, "read");
  vm_push_new_native(vm, "read", 2, &read_call);
  vm_push_string_from_chars(vm, -1, "write");
  vm_push_new_native(vm, "write", 2, &write_call);
  vm_push_string_from_chars(vm, -1, "readln");
  vm_push_new_native(vm, "readln", 1, &readln_call);
  vm_push_string_from_chars(vm, -1, "writeln");
  vm_push_new_native(vm, "writeln", 2, &writeln_call);
  ASSERT(vm_construct(vm, 20) == STATUS_OK, "cannot load library `io`");
}
